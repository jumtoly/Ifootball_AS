...
public class MyFileResponseCache extends FileResponseCache {
	private static final String TAG = "NeweggFileResponseCache";

    public static void install(Context context) {
        ResponseCache responseCache = ResponseCache.getDefault();
        if (responseCache instanceof MyFileResponseCache) {
            Log.d(TAG, "Cache has already been installed.");
        } else if (responseCache == null) {
        	MyFileResponseCache dropCache = new MyFileResponseCache(context);
    ...
    public MyFileResponseCache(Context context) {
        mContext = context;
    }
